...
func (d *DriverV2) NodeGetInfo(ctx context.Context, req *csi.NodeGetInfoRequest) (*csi.NodeGetInfoResponse, error) {
	instances, ok := d.controllerProvisioner.Cloud.Instances()
	if !ok {
	...
	zone, err := d.controllerProvisioner.Cloud.GetZone(ctx)
	if err != nil {
		klog.Warningf("Failed to get zone from Azure cloud provider, nodeName: %v, error: %v", d.NodeID, err)
	} else {
		if isAvailabilityZone(zone.FailureDomain, d.controllerProvisioner.Cloud.Location) {
